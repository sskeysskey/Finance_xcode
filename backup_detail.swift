    ...
    var body: some View {
        ...
        ScrollView {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                // 在 ArticleDetailView 的 body 中，替换原有的段落 Text 部分
                ForEach(paragraphs.indices, id: \.self) { pIndex in
                    // 使用 GeometryReader 来获取可用宽度
                    GeometryReader { geometry in
                        SelectableTextView(
                            text: paragraphs[pIndex],
                            font: UIFont(name: "NewYork-Regular", size: 21) ?? UIFont.systemFont(ofSize: 21),
                            lineSpacing: 14
                        )
                        .frame(
                            width: geometry.size.width - 36, // 36 = 18 * 2 (左右padding)
                            height: SelectableTextView.height(
                                for: paragraphs[pIndex],
                                font: UIFont(name: "NewYork-Regular", size: 21) ?? UIFont.systemFont(ofSize: 21),
                                lineSpacing: 14,
...
struct SelectableTextView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UITextView {
        ...
        textView.isScrollEnabled = false // 重要：防止内部滚动
    ...
    // 添加一个计算高度的辅助方法
    static func height(for text: String, font: UIFont, lineSpacing: CGFloat, width: CGFloat) -> CGFloat {
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.lineSpacing = lineSpacing

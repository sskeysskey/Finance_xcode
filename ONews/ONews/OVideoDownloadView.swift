    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        onMain {
            ...
            if didTrulyFinish {
                ...
                let metadata = self.cacheMetadata[urlString]
                let title = metadata?.title ?? "Unknown Video"
                ...
                // ⭐ 下载成功也触发追剧
                //
                // 必须使用剧集详情页 sourceURL，而不是解析后的真实播放地址 urlString。
                // SeriesTrackManager 内部会自动过滤 Movie，所以电影下载成功不会进入追剧。
                let trackTitle = metadata?.seriesTitle.flatMap {
                    $0.isEmpty ? nil : $0
                } ?? (title.components(separatedBy: " · ").first ?? title)

                let trackSourceURL = metadata?.sourceURL
                let trackCover = metadata?.coverImage
                let trackEpisodeName = metadata?.episodeName

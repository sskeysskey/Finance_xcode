...
class HLSDownloadManager: NSObject, ObservableObject, AVAssetDownloadDelegate {
    ...
    // 记录下载进度 (URL字符串 -> 进度 0.0~1.0)
    @Published var downloadProgress: [String: Double] = [:]
    // 记录已下载的本地书签数据 (URL字符串 -> Bookmark Data)
    @Published var localBookmarks: [String: Data] = [:]
    
    private let bookmarksKey = "SavedHLSBookmarks"
...
// MARK: - 3. 播放器视图 (封装 AVPlayerViewController)
        ...
        NavigationView {
            List(videos) { video in
                HStack {
                    ...
                    // 导航到播放器
                    NavigationLink(destination: PlayerView(videoURL: playURL)
                    ...
                    if isDownloaded {
                        Button(action: {
                            downloadManager.deleteDownload(urlString: video.url)
                        }) {
                            Image(systemName: "trash")
                                .foregroundColor(.red)
                        }
                        .buttonStyle(BorderlessButtonStyle()) // 防止触发 NavigationLink
                    } else if downloadManager.downloadProgress[video.url] == nil {
                        Button(action: {
                            downloadManager.startDownload(urlString: video.url, title: video.title)
                        }) {
